...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        nums = torch.arange(0, max_len, dtype=torch.float)[:, None]
        pows = torch.pow(10000, -torch.arange(0, embed_dim, 2) / embed_dim)

        pe[0, :, 0::2] = torch.sin(nums * pows)
        pe[0, :, 1::2] = torch.cos(nums * pows)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        Inputs:
         - embed_dim: Dimension of the token embedding - length of sequence, d
         - num_heads: Number of attention heads, d
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # compute parameter matrices from input data, for that reason we needed to init with
        # fc affine layer and multiply it with data
        ...
        Y = Q @ K.transpose(2,3) / np.sqrt(self.head_dim)

        if attn_mask is not None:
            Y = Y.masked_fill(attn_mask == 0, -np.inf)
