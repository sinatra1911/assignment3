...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions_embbeded = self.embedding(captions)
        captions_embbeded = self.positional_encoding(captions_embbeded)

        # projecting the features
        features_projected = self.visual_projection(features).unsqueeze(1)

        # preparing mask
        mask = torch.tril(torch.ones(T,T, dtype=captions_embbeded.dtype))

        # apply the transformer
        features = self.transformer(tgt=captions_embbeded, memory=features_projected, tgt_mask=mask)
