...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        pos_sim_left = sim(z_k, z_k_N) / tau
        pos_sim_right = sim(z_k_N, z_k) / tau

        all_sims_left = torch.stack([sim(z_k, z) for z in out]) / tau
        all_sims_right = torch.stack([sim(z_k_N, z) for z in out]) / tau

        all_sims_left[k] = -float('inf')
        all_sims_right[k + N] = -float('inf')

        l1 = - (pos_sim_left - torch.logsumexp(all_sims_left, dim=0))
        l2 = - (pos_sim_right - torch.logsumexp(all_sims_right, dim=0))
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (left * right).sum(dim=1, keepdims=True)
...
def compute_sim_matrix(out):
    ...
    normed = out / torch.linalg.norm(out, dim=1, keepdims=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exp = torch.exp(sim_matrix / tau).to(device)
    
    # This binary mask zeros out terms where k=i.
    mask = (torch.ones_like(exp, device=device) - torch.eye(2 * N, device=device)).to(device).bool()
    
    # We apply the binary mask.
    exp = exp.masked_select(mask).view(2*N, -1)
    ...
    pos_sim = sim_positive_pairs(out_left, out_right).to(device)
    pos_sim_2N = torch.cat([pos_sim, pos_sim], dim=0).squeeze()

    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(pos_sim_2N / tau)
